...
### 🛡️ Блок 2. Системная безопасность, привилегии и PowerShell IPC (Security, Permissions & Elevated Helper)
...
* **Уже исправлено в кодовой базе (проверено 951 тестом):**
  1. **Ликвидация небезопасного вызова командной строки (`src/main/autoconfig/androidStudio.ts`):** Поиск директорий Android Studio выполнялся через вызов `cmd.exe /c dir /b` с риском командной инъекции при наличии спецсимволов в путях. Заменено на нативный неблокирующий `fs/promises.readdir()`.
* **Приоритетные направления блока по результатам аудита:**
  * **Контроль за зависшими процессами elevated-хелпера (`src/main/elevatedPsHelper.ts`):** Когда в Node.js истекает таймаут ожидания ответа от повышенного (Admin) процесса PowerShell, промис в Node реджектится, но дочерний процесс PowerShell продолжает бессмысленно молотить в фоне с правами Администратора. Требуется явная передача cancellation token / Job Object или принудительный `taskkill /F /PID` зависшего воркера.
  * **Искоренение `Invoke-Expression` (`src/main/elevatedPsHelper.ts`):** Модель выявила 98% риск `high_risk_eval`. Выполнение произвольных динамических строк через `Invoke-Expression` в процессе с правами Администратора должно быть заменено на строго параметризованные `[ScriptBlock]` с валидацией входных аргументов.
  * **Строгое исполнение внешних утилит без шелла (`src/main/leakDiagnostics.ts`):** Вызовы диагностических утилит через `exec('curl.exe ...')` передаются через `cmd.exe`. Необходимо перевести на `execFile('curl.exe', [...args])` без вызова интерпретатора командной строки.
  * **Корректное экранирование аргументов в Windows CLI (`src/main/admin.ts`):** Использование Unix-стиля `\"` внутри командных строк Windows `cmd.exe` приводит к сбоям парсинга при наличии пробелов в путях; требуется стандартное для Windows удвоение кавычек `""`.
