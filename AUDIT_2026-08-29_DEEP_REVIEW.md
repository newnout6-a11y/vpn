...
| № | Находка | Объём правки |
|---|---|---|
| 1 | Runtime-каталог в `%APPDATA%` + всегда-админ = EoP | средний (ACL + верификация подписи) |
| 2 | Kill-switch снимается при смене сервера и ротации | **малый** — механизм уже есть |
| 3 | `http://ip-api.com` со списком серверов | **тривиальный** — один символ |
| 4 | Провал ротации оставляет систему без защиты молча | малый |
| 5 | Форензика в ZIP без редакции при заявленной редакции | средний |
| 6 | Потеря побочных эффектов оркестратора | средний (рефакторинг) |
| 7-9 | Денилист PS, двойное состояние kill-switch, дублирование | по обстоятельствам |
...
- **Приложение не запускалось.** Реальное поведение firewall, Wintun, маршрутов, DNS на живой Windows не подтверждено. Все выводы — из чтения кода.
- **Тесты не запускались** (`npm test`, `npm run typecheck`). Утверждения о покрытии основаны на именах и содержимом тестовых файлов, а не на прогоне.
